...
public class PALMDB {
            ...
            if (rs.next()) {
                for (String line : rs.getString("items").split("\n")) {
                    ...
                    switch (params[0]) {
                        ...
                        case "LoginWebSite" -> items.add(new LoginWebSite(params[1], params[2], params[3], params[4]));
                        case "LoginDevice" -> items.add(new LoginDevice(params[1], params[2], params[3], params[4]));
    ...
    /**
     * Lists all users contained in the database alphabetically and their encrypted information
     */
    public void listUsers() {

        // Count users
        String countQuery = "SELECT count(*) AS total FROM usersTable";
        int userCount = 0;
        try (Connection connection = connect();
        ...
        // Print all users
        String userQuery = "SELECT username FROM usersTable ORDER BY username ASC";
        try (Connection connection = connect();
             PreparedStatement ps = connection.prepareStatement(userQuery)) {
            ResultSet rs = ps.executeQuery();
            System.out.printf("- List of %3d users alphabetically -\n", userCount);
            System.out.println("------------------------------------");
            while (rs.next()) {
        ...
        String infoQuery = "SELECT usersTable.username, usersTable.salthash, itemsTable.items FROM usersTable " +
                           "INNER JOIN itemsTable ON itemsTable.username = usersTable.username";
        try (Connection connection = connect();
             PreparedStatement ps = connection.prepareStatement(infoQuery)) {
            ResultSet rs = ps.executeQuery();
            System.out.println("\n- User encrypted information -");
            System.out.println("------------------------------");
            while (rs.next()) {
                System.out.println(rs.getString("username"));
